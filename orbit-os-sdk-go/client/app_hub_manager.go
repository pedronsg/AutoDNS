...
// GetService returns the registration details for the given service ID.
func (m *AppHubManager) GetService(serviceID string) (*apphubv26.Service, error) {
	ctx, cancel := context.WithTimeout(m.ctx, 10*time.Second)
	defer cancel()
	resp, err := m.client.GetService(ctx, &apphubv26.ServiceIdRequest{ServiceId: serviceID})
	if err != nil {
		return nil, fmt.Errorf("GetService: %w", err)
	}
	if e := resp.GetError(); e != nil && e.GetMessage() != "" {
		return nil, &AppHubError{Code: e.GetCode().String(), Message: e.GetMessage()}
	}
	return resp.GetService(), nil
}

// ListServices returns all services currently registered in the portal.
func (m *AppHubManager) ListServices() ([]*apphubv26.Service, error) {
	ctx, cancel := context.WithTimeout(m.ctx, 10*time.Second)
	defer cancel()
	resp, err := m.client.ListServices(ctx, &types.Empty{})
	if err != nil {
...
func (m *AppHubManager) GetRoutingTable() ([]*apphubv26.RoutingEntry, error) {
	ctx, cancel := context.WithTimeout(m.ctx, 10*time.Second)
	defer cancel()
	resp, err := m.client.GetRoutingTable(ctx, &types.Empty{})
	if err != nil {
		return nil, fmt.Errorf("GetRoutingTable: %w", err)
	}
	if e := resp.GetError(); e != nil && e.GetMessage() != "" {
		return nil, &AppHubError{Code: e.GetCode().String(), Message: e.GetMessage()}
	}
...
// WatchServices streams service lifecycle events until ctx is cancelled or the server closes.
func (m *AppHubManager) WatchServices(ctx context.Context, handler func(*apphubv26.ServiceEvent)) error {
	stream, err := m.client.WatchServices(ctx, &types.Empty{})
	if err != nil {
		return fmt.Errorf("WatchServices: %w", err)
	}
	for {
		evt, err := stream.Recv()
		if err != nil {
			return fmt.Errorf("WatchServices recv: %w", err)
		}
		handler(evt)
	}
}
