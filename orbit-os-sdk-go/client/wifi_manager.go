...
func (w *WiFiManager) StartAP(ifname, ssid, password, band string, channel int32) (bool, error) {
	if _, err := w.client.SetAPConfig(w.ctx, &wifisvcv26.APConfig{
		...
		return false, fmt.Errorf("StartAP SetAPConfig(%s ssid=%s): %w", ifname, ssid, err)
	}
	resp, err := w.client.StartAP(w.ctx, &wifisvcv26.WiFiInterfaceRequest{
		InterfaceName: ifname,
	})
	if err != nil {
		return false, fmt.Errorf("StartAP(%s): %w", ifname, err)
	}
	if err := wifiErrCheck(resp.GetError()); err != nil {
		return false, err
	}
...
func (w *WiFiManager) StopAP(ifname string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("StopAP(%s): %w", ifname, err)
	}
	if err := wifiErrCheck(resp.GetError()); err != nil {
		return false, err
	}
