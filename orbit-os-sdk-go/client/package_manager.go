...
// ListInstalledPackages returns all packages installed on the device (from app_manager).
func (p *PackageManager) ListInstalledPackages() ([]*pmv26.InstalledPackage, error) {
...
// InstallPackage streams a local .orb file to Gravity via InstallUpdatePackage (chunked + MD5).
...
// packageID is the package_id as reported by ListInstalledPackages (InstalledPackage.PackageId).
